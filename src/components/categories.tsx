...
const Categories = () => {
  ...
  const normalizedPathname = pathname.startsWith(localePrefix) ? pathname.slice(localePrefix.length) || "/" : pathname

  const activeCategory =
    categories.find((item) => normalizedPathname === item.link || (normalizedPathname === "/" && item.link === "/")) ||
    categories[0]
              ...
              {categories.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive && <div className="ml-auto w-2 h-2 bg-[#01c89b] rounded-full"></div>}
                  </Link>
                )
              })}
            </div>
